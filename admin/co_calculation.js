document.addEventListener("DOMContentLoaded", () => {
  ...
  function fetchTestAssignmentData(fc_id) {
    fetch(`../hod/fetch_progress.php?fc_id=${fc_id}`)
      .then((response) => response.json())
      .then((data) => {
        ...
        const marksViewHTML = `
          ...
          testDetails.test_no || testDetails.assignment_no
              ...
              <table class="marks-table">
                <thead>
                  <tr>
                    ...
                    <th>Student Name</th>
                    ${questions
                      .map((q) => `<th>Q${q.question_no}<br>${q.max_mark}</th>`)
                      .join("")}
                    <th>Total</th>
                  </tr>
                </thead>
            ...
            <div class="marks-actions">
                <p>test_id=${testId}&test_type=${testType}</p>
              ${
                testDetails.status == "Freeze"
                  ? `<button class="btn print-btn" onclick="unfreezeTest(${testId}, '${testType}')">
              <i class="fa-solid fa-unlock"></i> Unfreeze
            </button>`
                  : ""
              }
  ...
  window.unfreezeTest = function unfreezeTest(testId, testType) {
    if (!confirm("Are you sure you want to unfreeze this test?")) return;

    fetch(`./unfreeze.php?test_id=${testId}&test_type=${testType}`)
      .then((response) => response.json())
      .then((data) => {
        if (data.success) {
          alert("Test unfreezed successfully!");
          location.reload();
        } else {
          alert("Failed to unfreeze: " + data.error);
        }
      })
      .catch((error) => {
        console.error("Error:", error);
        alert("An error occurred while unfreezing.");
      });
  };
