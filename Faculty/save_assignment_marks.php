...
try {
    ...
    $stmt = $conn->prepare("
            INSERT INTO assignment_mark (student_id, assignment_id, question_id, obtained_mark) 
            VALUES (?, ?, ?, ?)
    ...
    // Prepare the insert statement for co_results
    $coStmt = $conn->prepare("
            INSERT INTO co_assignment_results (co_id, assignment_id, co_percentage, co_level) 
            VALUES (?, ?, ?, ?)
    ...
    $testQuestionsStmt = $conn->prepare("SELECT q.id, q.target_mark, qc.co_id, co.co_number
                                         ...
                                         JOIN assignment_question_co qc ON q.id = qc.question_id
    ...
    foreach ($marksData as $mark) {
        ...
        $assignment_id = $mark['test_id'];
        $question_id = $mark['question_id'];
        $obtained_mark = $mark['obtained_mark'];

        $stmt->bind_param("siid", $student_id, $assignment_id, $question_id, $obtained_mark);
        if (!$stmt->execute()) {
    ...
    $testQuestionsStmt->bind_param("i", $assignment_id);
    ...
    foreach ($coGroups as $co_id => $group_data) {
        ...
        $coStmt->bind_param("iidd", $co_id, $assignment_id, $avgPercentage, $co_level);
        if (!$coStmt->execute()) {
